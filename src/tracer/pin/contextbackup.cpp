      ...
      triton::uint512 getCurrentRegisterValue(const triton::arch::Register& reg) {
        ...
        if (tracer::pintool::context::lastContext == nullptr)
          ...
          switch (reg.getParent()) {
            ...
            case triton::arch::ID_REG_X86_SS:      PIN_GetContextRegval(const_cast<const long unsigned int**>(tracer::pintool::context::lastContext), static_cast<REG>(REG_SEG_SS),       reinterpret_cast<triton::uint8*>(buffer)); break;
